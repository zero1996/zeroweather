...
//				locatiionCityTV.setText("正在定位...");
			...
			case Utils.MSG_LOCATION_FINISH:
				...
				String district = Utils.parseDistrictName(result.get("district"));
				String city = Utils.parseCityName(result.get("city"));
				String province = Utils.parseProvinceName(result.get("province"));
				String code = zeroWeatherDB.queryCityCode(district, province);
				//先通过区域名查询城市代码，查询不到则通过城市名查询。
				if("".equals(code)){
					code = zeroWeatherDB.queryCityCode(city, province);
//					tv.setText(city+":"+code);
				}else{
//					tv.setText(district+":"+code);
				}
				//获取天气
				getWeather(code);
				break;
		...
		ApiStoreSDK.execute("http://apis.baidu.com/heweather/weather/free",
			...
			public void onSuccess(int status, String responseString) {
				...
				if(weather != null){
					...
					todayTmpScope.setText(weather.getTodayTempMax()+"°/"+weather.getTodayTempMin()+"℃");
	...
	public Weather parseWeatherJson(String weatherJson){
		...
		try {
			...
			if(jsonObj.has("HeWeather data service 3.0")){
				JSONArray heWeather = jsonObj.getJSONArray("HeWeather data service 3.0");
